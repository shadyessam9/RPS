...
public class billviewe implements Initializable {
    @FXML
    private Label bd;
    @FXML
    private Label bid;
    ...
    private ObservableList<billviewe.ShowData> data;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        idb=transdetails.eid;
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    while (results.next()) {
                        data.add(new billviewe.ShowData(results.getString("item"),results.getString("quan"),results.getString("price")));
                    }
                ...
                try {
                    ...
                    while (results.next()) {
                        ...
                        try {
                            ...
                            statement.execute("SELECT * FROM suppliers WHERE id='"+results.getString("sup_id")+"'");
                            ResultSet results1 = stmt.getResultSet();
                            while (results1.next()){
                                sid=results.getString("name");
